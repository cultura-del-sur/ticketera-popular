      ...
      {tickets.map((ticket, index) => (
        <div key={index} className="border-b border-zinc-200 pb-4 last:border-b-0">
          <div className="flex items-center justify-between mb-2">
            <span className="font-semibold">{ticket.type_label}</span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center text-zinc-400">
              ...
              {ticket.available_tickets === 0 ? (
                ...
                <span>{ticket.available_tickets} tickets disponibles</span>
              )}
            </div>
            {ticket.available_tickets > 0 && (
              <Link
                href={`/events/${eventId}/buy-tickets?ticketType=${encodeURIComponent(ticket.type_slug)}`}
